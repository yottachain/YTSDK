            ...
            try {
                ...
                if (LOG.isDebugEnabled()) {
                    if (resp.getRES() == UploadShardRes.RES_OK || resp.getRES() == UploadShardRes.RES_VNF_EXISTS) {
                        LOG.debug("[" + VNU + "]Upload OK:" + req.getVBI() + "/(" + req.getSHARDID() + ")"
                                + Base58.encode(req.getVHF()) + " to " + node.getId() + ",RES:"
                                + resp.getRES() + ",take times " + (System.currentTimeMillis() - l) + " ms");
                    } else {
                        LOG.error("[" + VNU + "]Upload ERR:" + req.getVBI() + "/(" + req.getSHARDID() + ")"
                                + Base58.encode(req.getVHF()) + " to " + node.getId() + ",RES:"
                                + resp.getRES() + ",take times " + (System.currentTimeMillis() - l) + " ms");
                    }
                }
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.error("[" + VNU + "]Upload ERR:" + req.getVBI() + "/(" + req.getSHARDID() + ")"
                            + Base58.encode(req.getVHF()) + " to " + node.getId() + ",take times " + (System.currentTimeMillis() - l) + " ms");
                }
            }
