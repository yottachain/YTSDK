...
public class UploadFackBlockExecuter implements Runnable {

    private static final Logger LOG = Logger.getLogger(UploadFackBlockExecuter.class);
    private static LinkedBlockingQueue<UploadFackBlockExecuter> BLOCK_POOL = null;

    public static void init() {
        if (BLOCK_POOL == null) {
            BLOCK_POOL = new LinkedBlockingQueue(UserConfig.UPLOADBLOCKTHREAD);
            for (int ii = 0; ii < UserConfig.UPLOADBLOCKTHREAD; ii++) {
                BLOCK_POOL.add(new UploadFackBlockExecuter());
            }
        }
    }

    static void startUploadBlock(UploadFackObject uploadObject, Block b, short ii) throws InterruptedException {
        UploadFackBlockExecuter uploader = BLOCK_POOL.take();
    ...
    private UploadFackObject uploadObject;
    private Block b;
